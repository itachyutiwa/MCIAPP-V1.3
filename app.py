...
import os
import numpy as np
from models import (Production, Prestation)
...
def critere_benef(file,ante=pd.to_datetime(str(start_date_entry.get())), post=pd.to_datetime(str(end_date_entry.get()))):
    ...
    if len(benef_sans_date_naiss) != 0:
        path_benef_sans_date_naiss = os.path.join(nom_dossier, sous_dossier, "BENEF SANS DATE DE NAISSANCE.xlsx")
        benef_sans_date_naiss.to_excel(path_benef_sans_date_naiss)

    #------------Controle production 
    Obj = Production().database(selected_beneficiaire_path,selected_emission_path,selected_consommation_path)['infos']
    aff_avnt = Production().affiliation_avnants(selected_beneficiaire_path,selected_emission_path,selected_consommation_path)
    aff_pieces_justif = Production().affiliation_pieces_justificatives(selected_beneficiaire_path,selected_emission_path,selected_consommation_path,n_alea=3)
    aff_facuration = Production().affiliation_facuration(selected_beneficiaire_path,selected_emission_path,selected_consommation_path,n_alea=4)
    recouvrement = Production().recouvrement(selected_beneficiaire_path,selected_emission_path,selected_consommation_path)
    facturation_frais_gestion = Production().facturation_frais_gestion(selected_beneficiaire_path,selected_emission_path,selected_consommation_path)
    rs = Production().resultats(selected_beneficiaire_path,selected_emission_path,selected_consommation_path)
    ...
    if len(df_production) != 0:
        path_df_production = os.path.join(nom_dossier, sous_dossier, "PRODUCTION.xlsx")
        df_production.to_excel(path_df_production)
    return
...
def critere_conso(file):
        ...
        #------------Controle production 
        Obj = Prestation().database(selected_beneficiaire_path,selected_emission_path,selected_consommation_path)['infos']
        rs = Prestation().resultats(selected_beneficiaire_path,selected_emission_path,selected_consommation_path,date_fin=end_date_entry.get())
        ...
        if len(df_prestation) != 0:
            path_df_prestation = os.path.join(nom_dossier, sous_dossier, "PRESTATION.xlsx")
            df_prestation.to_excel(path_df_prestation)
        return
...
def resume(file_benef, file_emission, file_conso, seuil_ctx,ante=pd.to_datetime(str(start_date_entry.get())), post=pd.to_datetime(str(end_date_entry.get()))):
    ...
    if len(benef_sans_date_naiss) != 0:
        try:
            condition = sans_doublons_conso['Matricule Beneficiaire'].isin(benef_sans_date_naiss["Matricule"].unique())
